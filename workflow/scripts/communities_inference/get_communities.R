...
if(kwp_correct){
  ...
  #msg("Something went wrong... quitting\n!")
  #quit()
  kwp_max_com <- max(kwp_communities$com)
  missing_final <- cl_cat %>% dt_filter(category == "KWP") %>% mutate(cl_name=as.character(cl_name)) %>%
   filter(!cl_name %in% kwp_communities$cl_name) %>% mutate(com=kwp_max_com + row_number())
  kwp_communities <- bind_rows(kwp_communities,missing_final)
  kwp_n_comp <- kwp_communities$com %>% unique() %>% length()
  kwp_n_clus <-  kwp_communities$cl_name %>% unique() %>% length()
}
...
if(gu_correct){
  ...
  #msg("Something went wrong... quitting\n!")
  #quit()
  gu_max_com <- max(gu_communities$com)
  missing_final <- cl_cat %>% dt_filter(category == "GU") %>% mutate(cl_name=as.character(cl_name)) %>%
   filter(!cl_name %in% gu_communities$cl_name) %>% mutate(com=gu_max_com + row_number())
  gu_communities <- bind_rows(gu_communities,missing_final)
  gu_n_comp <- gu_communities$com %>% unique() %>% length()
  gu_n_clus <-  gu_communities$cl_name %>% unique() %>% length()
}
        ...
        if (cfg$eu_gc != ""){
          ...
          if (eu_mids_l > 0){
            ...
            if(eu_correct){
              ...
              #msg("Something went wrong... quitting\n!")
              #quit()
              eu_max_com <- max(eu_communities$com)
              missing_final <- cl_cat %>% dt_filter(category == "EU") %>% mutate(cl_name=as.character(cl_name)) %>%
               filter(!cl_name %in% eu_communities$cl_name) %>% mutate(com=eu_max_com + row_number())
              eu_communities <- bind_rows(eu_communities,missing_final)
              eu_n_comp <- eu_communities$com %>% unique() %>% length()
              eu_n_clus <-  eu_communities$cl_name %>% unique() %>% length()
            }
