...
--slots  number of CPU-cores to use" 1>&2
  exit 1
}

OPT_LIST="derep:,msa:,msaeval:,ssn:,gfilter:,gconnect:,seqp:,datap:,stats:,outdir:,slots:"
...
function dereplicate() {
  ...
  "${MMSEQS_BIN}" clusthash "${DB}" "${REPLIC}" --min-seq-id 0.90 --threads "${NSLOTS}" &>/dev/null
  "${MMSEQS_BIN}" clust "${DB}" "${REPLIC}" "${REPLIC}"_clust --threads "${NSLOTS}" &>/dev/null
  "${MMSEQS_BIN}" createtsv "${DB}" "${DB}" "${REPLIC}"_clust "${N}"_replicates.tsv --threads "${NSLOTS}" &>/dev/null
  "${MMSEQS_BIN}" result2repseq "${DB}" "${REPLIC}"_clust dedup_"${N}"_rep --threads "${NSLOTS}" &>/dev/null
