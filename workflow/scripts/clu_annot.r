...
annot <- fread(opt$pfam_annot, stringsAsFactors = F, header = F, sep="\t") %>%
 setNames(c("orf", "name", "acc", "clan")) %>% mutate(orf=as.character(orf))
# Clustering results
clu <- fread(opt$clusters, stringsAsFactors = F, header = F,sep="\t") %>%
 setNames(c("cl_name","rep", "orf")) %>% mutate(orf=as.character(orf))
