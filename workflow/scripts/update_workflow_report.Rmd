...
genes <- fread(paste0(dir,"/gene_prediction/orf_partial_info.tsv"),
 stringsAsFactors = F,header = F) %>% select(V1)
...
```{r,echo=FALSE,warning=FALSE}
# Add new clusters...divsion original - shared - name_data
origin <- fread(paste0(dir,"/integrated_cluster_DB/cluDB_name_origin_size.tsv.gz"),stringsAsFactors = F,header = F,sep="\t")

tibble(Total_number_of_clusters=dim(origin)[1],
       Total_number_of_genes=sum(as.numeric(origin$V3)),
       Original = dim(origin %>% filter(!grepl(name_data,V2)))[1],
       Shared = dim(origin %>% filter(V2!=name_data & grepl(name_data,V2)))[1],
       New = dim(origin %>% filter(V2==name_data))[1]) %>% kable(digits=3, format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"),full_width = F)

```

The new clusters and either
- the previous singletons that are now shared cluster with more than 1 gene
- or all the existing gene clusters that got integrated with new genes (eval_shared=true),
will be processed through the workflow. The new dataset that will be validated and classified is described in the following table:

```{r,echo=FALSE}
# Then the new set that is evaluated: cluster - singletons
clu <- fread(paste0(dir,"/mmseqs_clustering/cluDB_no_singletons.tsv"),stringsAsFactors = F,header = F)
singl <- fread(paste0(dir,"/mmseqs_clustering/cluDB_singletons.tsv"),stringsAsFactors = F,header = F)

tibble(Total_number_of_clusters=dim(clu %>% select(V1) %>% distinct())[1] + dim(singl)[1],
       Singletons = dim(singl)[1],
       Clusters = dim(clu %>% select(V1) %>% distinct())[1],
       Cluster_genes = dim(clu)[1]) %>% kable(digits=3, format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"),full_width = F)

```

```{r,echo=FALSE,results='asis'}
avg <- mean(clu %>% group_by(V1) %>% count() %>% .$n)
...
tibble(Type=c("Clusters","Genes"),
       ...
       Not_annotated_clusters=c(dim(clu %>% group_by(V1) %>% count() %>% filter(n>1))[1]-cl_annot,
       sum(clu %>% group_by(V1) %>% count() %>% filter(n>1) %>% .$n)-orf_annot)) %>% kable(digits=3, format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"),full_width = F)
...
tibble(Type=c("Number of clusters","Proportion of clusters","Number of genes","Proportion of genes"),
       Refined=c(length(unique(ref$V1)),length(unique(ref$V1))/val$good_cl_n,
       dim(ref)[1],dim(ref)[1]/val$good_cl_orfs),
       Discarded=c(val$good_cl_n-length(unique(ref$V1)),(val$good_cl_n-length(unique(ref$V1)))/val$good_cl_n,
                  val$good_cl_orfs-dim(ref)[1], (val$good_cl_orfs-dim(ref)[1])/val$good_cl_orfs)) %>% kable(digits=3, format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"),full_width = F)
