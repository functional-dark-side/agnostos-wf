...
option_list <- list(
  ...
  make_option(c("-x", "--plots"), type="character", default=1,
              help="validation plots", metavar="character")
)

opt_parser <- OptionParser(option_list=option_list)
opt <- parse_args(opt_parser)

if (is.null(opt$fval_res) |
    is.null(opt$cval_res) | is.null(opt$val_annot)|
    is.null(opt$val_res) | is.null(opt$val_stats) |
    is.null(opt$good) | is.null(opt$plots)){
...
save(cl_val_df, p_rej_cl_bsm,p_desc, p_rej_clp, p_panel, file=opt$plots)
