...
option_list <- list(
  ...
  make_option(c("-s", "--stage"), type="character", default=NULL,
              help="new data stage", metavar="character"),
...
if (is.null(opt$basedir) | is.null(opt$outdir) | is.null(opt$wf_report) |
    is.null(opt$input) | is.null(opt$stage) | is.null(opt$output)){
...
rmarkdown::render(opt$wf_report,
                  params = list(directory = opt$basedir, input_data=opt$input, stage=opt$stage),
