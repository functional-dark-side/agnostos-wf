...
tibble(Type=c("Number of clusters","Percentage of clusters","Number of genes","Percentage of genes"),
       Refined=c(length(unique(ref$V1)),length(unique(ref$V1))/val$total_clusters,dim(ref)[1],dim(ref)[1]/val$total_orfs*100),
       Discrded=c(val$good_cl_n-length(unique(ref$V1)),(val$good_cl_n-length(unique(ref$V1)))/val$total_clusters*100,
                  val$good_cl_orfs-dim(ref)[1], (val$good_cl_orfs-dim(ref)[1])/val$total_orfs*100)) %>% kable(digits=3, format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"),full_width = F)
