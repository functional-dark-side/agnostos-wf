rule cluster_db_results:
    ...
    shell:
        ...
        gzip -c {params.or_clu_origin} > {params.clu_origin}

        # All gene headers and partiality information
        gzip -c {params.or_partial}  > {params.ipartial}
        ...
        mv {params.or_clu_gene > {params.clu_gene}
        ...
        mv {input.iclu_hmm}* {params.or_clu_hmm}

        # New integarted cluster sequence DB (MMseqs sequence database with clustered index)
        mv {params.or_clu_seq} {params.clu_seq}
        mv {params.or_clu_seq}.index {params.clu_seq}.index
        mv {params.or_clu_seq}.dtype {params.clu_seq}.dbtype
