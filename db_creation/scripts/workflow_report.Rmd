...
contig_seqs <- system(paste0("cat ", input_data, "/*_contigs.fasta", " | grep -c '^>'"),intern = T)
cat(contig_seqs)
```

### Gene prediction

The gene were predicted using the Prodigal (Prokaryotic Dynamic Programming Gene-finding Algorithm) (version 2.6.3: February 2016) program [[1]](#1), applied in metagenomic mode.
The total number of predicted genes can be found in the following table.

```{r,echo=FALSE}
genes <- system(paste0("cat ",dir,"/combine_samples/orf_seqs.txt", " | wc -l "),intern = T)
...
```{r,echo=FALSE}
pfam_annot <- fread(paste0(dir,"/annot_and_clust/pfam_multi_annot_name_acc_clan.tsv"),stringsAsFactors = F,header = F)
...
We define a set of High Quality clusters, i.e. cluster with a high percentage of complete genes (>30%), and a complete representative.
